...
*   **功能**: 为管理员提供一个中心化的界面，用于对平台所有用户进行精细化的管理和控制。
*   **交互与特性**:
    *   以表格形式列出所有用户，包含其头像、姓名、邮箱、当前角色、状态和星级评分。
    *   **角色分配**: 管理员可以通过每行内的下拉菜单，动态地为用户分配新的角色（普通用户、创意者、供应商、管理员）。
    *   **状态管理**: 通过“更多操作”菜单，可对用户执行多种操作：
        *   **设为暂停**: 临时禁止用户登录和操作。
        *   **加入黑名单**: 永久封禁用户。
        *   **重新激活**: 解除暂停或黑名单状态。
        *   **删除用户**: 从系统中移除用户数据。
    *   **状态可视化**: 用户的“正常”、“已暂停”、“黑名单”等状态会通过不同颜色的徽章清晰展示。
    *   **星级评分**: 以1-5颗星的形式直观展示用户的信用或活跃度评级。
