...
export async function aiRequirementsNavigator(input: RequirementsNavigatorInput): Promise<RequirementsNavigatorOutput> {
  ...
  console.log("AI Requirements Navigator is currently disabled due to dependency conflicts.");
  // Return a mock response
  const lastMessage = input.conversationHistory[input.conversationHistory.length - 1];
  if (lastMessage?.parts[0]?.text.includes("确认")) {
      return {
  ...
  return {
    response: `(模拟回复) 感谢您的信息。关于您提到的“${lastMessage?.parts[0]?.text}”，能否再详细说明一下您的具体目标是什么？`,
    isFinished: false,
  };
}
