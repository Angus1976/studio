...
const requirementsNavigatorFlow = ai.defineFlow(
  {
    name: 'requirementsNavigatorFlow',
    inputSchema: RequirementsNavigatorInputSchema,
    outputSchema: RequirementsNavigatorOutputSchema,
  },
  async ({ conversationHistory }) => {
    const systemPrompt = `你是一个“AI需求导航器”，你的角色是通过一次友好、专业的对话，引导用户明确他们的业务需求。

你的目标是收集以下四个关键信息：
1.  **企业特征**: 用户的公司是做什么的？（例如：互联网、制造业、零售业）
2.  **用户角色**: 用户在公司中扮演什么角色？（例如：市场经理、软件开发、人事）
3.  **需求场景**: 用户希望在哪个具体的工作场景中使用AI？（例如：招聘流程、社交媒体营销、代码调试）
4.  **核心需求**: 在这个场景中，最核心、最具体的需求是什么？（例如：从简历中快速筛选候选人、为新产品写推广文案、解释一段复杂的代码）

**对话策略:**
- **主动引导**: 不要一次性问出所有问题。一次只问一个或两个相关的问题，逐步深入。
- **自然对话**: 你的语气应该是循循善诱的、乐于助人的。
...
- **默认开场白**: 如果对话历史为空，你的第一句话必须是：“您好！我是您的AI需求导航器。为了给您推荐最合适的AI能力，我需要先了解一些您的基本信息。请问，您所在的企业主要从事哪个行业呢？”`;
    ...
    // If conversation is empty, start with the greeting.
    ...
    const llmResponse = await ai.generate({
        ...
        output: {
            schema: RequirementsNavigatorOutputSchema,
        },
        config: {
            temperature: 0.5,
        }
