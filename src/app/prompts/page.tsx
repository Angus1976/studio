...
import { User, Settings, AlertTriangle, FileText, Edit, Trash2, Eye } from "lucide-react";
...
export default function PromptsPage() {
    const { user } = useAuth();
    const [selectedPrompt, setSelectedPrompt] = useState<typeof promptLibrary[0] | null>(null);
    ...
    const formatPromptForDialog = (prompt: typeof promptLibrary[0] | null) => {
        if (!prompt) return null;
        return {
            title: prompt.name,
            description: `ID: ${prompt.id}`,
            details: {
                "功能简述": prompt.description,
                "生效范围": prompt.scope,
                "状态": prompt.status === 'active' ? '生效中' : '已停用',
    ...
    return (
        <>
            ...
            <ItemDetailsDialog
                ...
                item={formatPromptForDialog(selectedPrompt)}
                isCode={true}
            />
        </>
    )
