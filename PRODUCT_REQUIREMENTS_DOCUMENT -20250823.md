...
*   **核心逻辑**:
    ...
    *   用户在相应的独立注册/登录页面中进行注册登录，四个角色：平台方管理员、工程师、企业租户、个人用户，平方方管理员角色超级用户权限为系统生成，不支持注册，仍支持平台方添加普通管理员，其他角色用户注册需要通过平台方管理员审核通过后方可生效。
   
*   **用户界面**:
    *   页面中央是品牌Logo和标题。
    *   按“平台方”和“用户方”分不同独立的登录注册页，每个角色一个带图标的按钮。
    *   再下方是传统的邮箱/密码/角色/登录表单。支持中国区手机号注册/登录及验证，并支持中国区短信验证平台API对接，如阿里云短信验证平台对接。
    *   页面底部提供切换到注册或登录页的链接。

### 4.2. AI 需求导航器 (主页核心组件)
*   **技术实现**:
    *   前端: `RequirementsNavigator` React 组件。
    *   后端: `aiRequirementsNavigator` Genkit Flow (`src/ai/flows/ai-requirements-navigator.ts`)，使用 `gemini-1.5-flash` 模型以及从平台方资源管理配置库中选择合适的全球大模型资源，并完成全球主流大模型和对应版本的API对接，平台和用户只需要添加相关API KEY即可使用，具体使用哪个模型可根据智能路由策略（可根据用户画像、需求分析、模型能力、Token单价等综合分析，手动设计权重，从页实现智能路由）设置，并需要完成相关账单统计对账和计费管理、发票管理。
*   **核心逻辑**:
    *   **引导式对话**: Flow 的 Prompt 被精心设计，以引导 AI 主动提问，逐步收集用户画像所需的四个关键信息：企业特征、用户角色、需求场景、核心需求等，并支持平台方人工手动不断调优设置。
    *   **上下文维持**: 每次交互都将完整的对话历史传递给 Genkit Flow或智能路由选择的合适大模型，确保 AI 能够理解上下文。可选记忆模式，以扩充、归纳总结上下文长度。
    *   **完成判断**: Prompt 指示 AI 在收集到所有信息后，进行总结，并询问用户确认。用户确认后，将 `isFinished` 标志位设为 `true`。
    *   **智能路由**: 完成后，AI 根据分析的用户需求，将 `suggestedPromptId` 设置为最匹配的专家ID（如 `recruitment-expert`）。此过程支持平台方管理员通过界面不断调参调优。
*   **用户界面**:
...
*   **核心逻辑**:
    ...
    *   **资源与权限**:
        *   提供管理企业自己账户的 API 密钥的入口。并完成全球主流大模型（包含中国的阿里、腾讯、DEEPSEEK等）和对应版本的API对接，平台和用户只需要添加相关API KEY即可使用，
...
*   **技术实现**:
    ...
    *   后端: `taskDispatch` Genkit Flow, 一个高级的AI流程。并完成全球主流大模型和对应版本的API对接，平台和用户只需要添加相关API KEY即可使用，
*   **核心逻辑**:
...
*   **可扩展性**: 平台架构应支持方便地增加新的 AI 模型、能力场景和第三方服务。Genkit Flow 和全球主流大模型和对应版本的API对接，平台和用户只需要添加相关API KEY即可使用，的设计应模块化，便于复用和扩展。
*   **安全性**:
...
- **AI/后端**: 全球所有的主流大模型，包含中国的大模型资源库中所配置可用的大模型，包括但不限于Genkit (Google AI), Firebase (V2: Firestore, Auth)
- **图标**: `lucide-react`
