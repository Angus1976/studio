...
*   **技术实现**:
    ...
    *   后端: `aiRequirementsNavigator` Genkit Flow (`src/ai/flows/ai-requirements-navigator.ts`)。后端流程设计已实现模型解耦，当前默认使用 `gemini-1.5-flash` 模型，但架构上已为未来的智能路由和多模型切换（如 PRD 中定义的全球主流 LLM）做好准备。
*   **核心逻辑**:
...
*   **技术实现**:
    ...
    *   后端: `taskDispatch` Genkit Flow (`src/ai/flows/task-dispatch-flow.ts`)。后端流程设计已实现模型解耦，当前默认使用 `gemini-1.5-flash` 模型，但架构上已为未来的智能路由和多模型切换（如 PRD 中定义的全球主流 LLM）做好准备。
*   **核心逻辑**:
